...
# 1 "Source/nes_gamepad.h" 1
# 24 "Source/nes_gamepad.h"
...
uint8_t last_gamepad_reading;


void __attribute__((picinterrupt(("")))) ISRCode (void)
{
...
void PrepareTxBuffer(uint8_t gamepad_reading)
{
    ...
    {
        do { PORTAbits.RA4 = 1; } while(0);

        int8_t x = 0;
        int8_t y = 0;

        if ((gamepad_reading & (1<<4)) > 0)
        {
            y = -128;
        }
        else if ((gamepad_reading & (1<<5)) > 0)
        {
            y = 127;
        }

        if ((gamepad_reading & (1<<6)) > 0)
        {
            x = -128;
        }
        else if ((gamepad_reading & (1<<7)) > 0)
        {
            x = 127;
        }
...
void ProcessIO(void)
{
    ...
    if (((DeviceState == 0x05) && (UCONbits.SUSPND==0))) CheckUsb();
